...
import { getStore,getuserDetails,setStore,setStoreSingle,getStoreSingle} from '../../auth/functions';
...
class DashboardPage extends Component<Props, State> {
    ...
    componentDidMount() {
        ...
        localStorage.getItem('countDown')==null ? localStorage.setItem('countDown',"true") :'' ;
        localStorage.getItem('openEditor')==null ? localStorage.setItem('openEditor',"true") :'' ;
        localStorage.getItem('launch')==null ? localStorage.setItem('launch',"true") :'' ;
    ...
    openPage(action,id=null) {
        if(action == 'openFile') {
           ...
           ipcRenderer.send('open-child', 'CreateWorkspace','Share Via  Email', 443, 315, false, false,id );
        }
    ...
    record(){
        //alert('dd');
        ipcRenderer.send('open-child', 'recording','Record', 750, 420, true, false );
    }
                           ...
                           return (<div className="main-container flexbox">
                                ...
                                <div className="right-hold">
                                    ...
                                    <Dropdown.Menu className='dropdown-xl'>
                                        <Dropdown.Item onClick={() =>{ this.openPage('copyLink',`https://adilo.bigcommand.com/watch/${wp.stored_name}`)}} href="#"><b>Copy link</b></Dropdown.Item>
                                        {/* <Dropdown.Item onClick={() =>{ this.openPage('senEmaillink',wp.id)}} href="#">Share via email</Dropdown.Item> */}
                                        <Dropdown.Item onClick={() =>{ this.openPage('openFile',`https://adilo.bigcommand.com/video/${wp.stored_name}`)}} href="#">Open in browser</Dropdown.Item>
                                        <Dropdown.Item onClick={() =>{ this.openPage('openFile',`https://adilo.bigcommand.com/projects/${wp.project.project_id}/edit/${wp.stored_name}`)}} href="#">Open in editor</Dropdown.Item>
                                        <Dropdown.Item onClick={() =>{ this.openPage('deleteFile',wp.id)}}  className='text-danger' href="#">Delete file</Dropdown.Item>
                                    </Dropdown.Menu>
                                    </Dropdown>
                                    </div></span>
                                </div>
